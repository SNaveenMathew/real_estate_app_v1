...
/* ── Map layers ─────────────────────────────────────────────────────────────── */
/* The Houses layer (this file, above) plus every other map layer — Crime, Bike Routes, Risk (NRI),
   Population, Sold Homes, Commute, and anything approved on the Data page — are all built and toggled
   by static/layers.js, which reads the catalog-driven layer list from /api/layers. It reaches into
   this file for two things only: `map` and `clusterGroup` (to show/hide the Houses layer), the same
   way commute.js already reaches into `map` and `state`. */
